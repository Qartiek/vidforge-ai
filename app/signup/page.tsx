...
export default function Signup() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!r.ok) {
        setError(d.error || "Unable to create your account. Please try again.");
        return;
      }
    ...
    <main className="auth-page">
      <div className="auth-shell">
        <section className="auth-copy">
          ...
          <h1 className="auth-title">Your studio.<br /><span className="auth-gradient">Powered by AI.</span></h1>
          <p className="auth-subtitle">Turn one idea into research, hook-led scripts, visuals, voiceovers, edited videos, SEO and platform-ready content.</p>
        </section>
        <form className="auth-card" onSubmit={submit}>
          <h2>Create your account</h2>
          <p className="auth-muted">Start building with VidForge AI.</p>
          <label className="auth-label" htmlFor="name">Name</label>
          <input id="name" className="auth-input" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" autoComplete="name" />
          ...
          <p className="auth-switch">Already have an account? <a className="auth-link" href="/login">Sign in</a></p>
        </form>
      </div>
    </main>
  );
