...
const settings = [
  ["Profile", "Account, name and preferences"],
  ["Appearance", "Theme, density and interface"],
  ["AI Preferences", "Models, creativity and defaults"],
  ["Video Defaults", "Format, quality and duration"],
  ["Voice & Captions", "Audio, voice and subtitle defaults"],
  ["Brand Kit", "Logo, colors and brand assets"],
  ["Notifications", "Email and product alerts"],
  ["Integrations", "YouTube, AI, storage and providers"],
  ["Privacy & Security", "Sessions, password and privacy"],
  ["Billing & Usage", "Plan, usage and invoices"],
  ["Developer", "API and automation settings"],
];
...
export default async function Dashboard() {
  ...
  return <main className="container dashboard-page">
    ...
    <div className="dashboard-layout">
      <aside className="dashboard-sidebar" aria-label="NOVYN navigation">
        <div className="sidebar-title">WORKSPACE</div>
        <a className="sidebar-link active" href="/dashboard">⌂ <span>Dashboard</span></a>
        <a className="sidebar-link" href="/create">✦ <span>Create Video</span></a>
        <a className="sidebar-link" href="/production">▣ <span>My Videos</span></a>
        <a className="sidebar-link" href="/templates">◇ <span>Templates</span></a>
        <a className="sidebar-link" href="/research">⌕ <span>Research</span></a>
        <a className="sidebar-link" href="/packaging">◈ <span>Viral Packaging</span></a>
        <a className="sidebar-link" href="/production">◉ <span>Production</span></a>
        <a className="sidebar-link" href="/schedule">◷ <span>Schedule</span></a>
        <a className="sidebar-link" href="/production">⌁ <span>Analytics</span></a>
        <a className="sidebar-link" href="/providers">⚡ <span>Integrations</span></a>
        <div className="sidebar-divider"/>
        <div className="sidebar-title">SYSTEM</div>
        <a className="sidebar-link settings-link" href="#settings">⚙ <span>All Settings</span></a>
        <a className="sidebar-link" href="/pricing">◇ <span>Billing & Usage</span></a>
        <div className="sidebar-status"><span className="status-dot"/><div><strong>System operational</strong><small>All core services online</small></div></div>
      </aside>
      ...
      <div className="dashboard-content">
        ...
        <section id="settings" className="workspace settings-panel"><div className="workspace-head"><div><span className="eyebrow">SYSTEM CONTROL</span><h2>All NOVYN settings</h2></div><span className="live-pill"><span/> Protected</span></div><div className="settings-grid">{settings.map(([title,description])=><a href="/providers" className="settings-card" key={title}><span className="settings-icon">⚙</span><span><strong>{title}</strong><small>{description}</small></span><b>→</b></a>)}</div><p className="hint">Settings are grouped here so the workspace stays clean on every screen size. Feature-specific controls open inside their respective modules.</p></section>
