...
const allowedStages = ["voice", "visuals", "thumbnail", "seo", "render", "publish"] as const;
...
export async function POST(request: Request) {
  ...
  if (!projectId || !scriptId || !stage || !allowedStages.includes(stage)) {
    return NextResponse.json({ error: "projectId, scriptId and a valid stage are required" }, { status: 400 });
  }
  ...
  try {
    ...
    await db.auditLog.create({
      data: {
        userId: user.id,
        action: `PIPELINE_${stage.toUpperCase()}`,
        resource: "Project",
        resourceId: projectId,
        success: true,
        metadata: JSON.stringify({ scriptId }),
      },
    });
    ...
    await db.auditLog.create({
      data: {
        ...
        resourceId: projectId,
        success: false,
        metadata: JSON.stringify({ scriptId, error: message.slice(0, 500) }),
      },
    }).catch(() => undefined);
