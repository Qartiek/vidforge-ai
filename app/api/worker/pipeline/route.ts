...
export async function POST(request: Request) {
  ...
  if (job.status === "RUNNING") {
    if (!job.startedAt || Date.now() - job.startedAt.getTime() < STALE_AFTER_MS) return NextResponse.json({ status: "RUNNING", jobId: job.id }, { status: 409 });
  ...
  try {
    ...
    const automation = payload.metadata?.automationRunId ? { runId: String(payload.metadata.automationRunId), autoPublish: payload.metadata.autoPublish === true, privacyStatus: payload.metadata.privacyStatus === "public" || payload.metadata.privacyStatus === "unlisted" ? String(payload.metadata.privacyStatus) : "private" } : null;

    if (stage === "publish") {
      const publishResult = result as { publishId?: string };
      if (automation?.autoPublish && publishResult.publishId) {
        const existingUpload = await db.job.findFirst({ where: { projectId: payload.projectId, userId: job.userId, type: "YOUTUBE_PUBLISH", status: { in: ["QUEUED", "RUNNING", "SUCCEEDED"] } }, orderBy: { createdAt: "desc" } });
        if (!existingUpload) {
          const publish = await db.youTubePublish.findFirst({ where: { id: publishResult.publishId, userId: job.userId } });
          if (!publish) throw new Error("AutoPilot publish record not found");
          if (publish.privacyStatus !== automation.privacyStatus) await db.youTubePublish.update({ where: { id: publish.id }, data: { privacyStatus: automation.privacyStatus } });
          const uploadJob = await db.job.create({ data: { userId: job.userId, projectId: payload.projectId, type: "YOUTUBE_PUBLISH", status: "QUEUED", payload: JSON.stringify({ publishId: publish.id, approvedAt: new Date().toISOString(), automationRunId: automation.runId }) } });
          nextJobId = uploadJob.id;
        } else nextJobId = existingUpload.id;
      ...
      const created = await enqueuePipelineStage(job.userId, { projectId: payload.projectId, scriptId: payload.scriptId, stage: next, previousJobId: job.id, metadata: payload.metadata });
