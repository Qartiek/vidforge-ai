...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (publish.status === "PUBLISHED") {
      ...
      return NextResponse.json({ status: "PUBLISHED", publishId, youtubeVideoId: publish.youtubeVideoId });
    }

    // Once YouTube upload starts, a network failure can be ambiguous: YouTube
    // may have accepted the bytes even when our request timed out. Do not
    // automatically retry an in-flight upload, otherwise a second video can
    // be created. Retries remain enabled for failures before upload begins.
    ...
    const videoId = await uploadYouTubeVideo(access.accessToken, {
      file,
      title: publish.title,
    ...
    await db.youTubePublish.update({
      where: { id: publish.id },
      data: { status: "PUBLISHED", youtubeVideoId: videoId, publishedAt: new Date(), error: null },
    });
    ...
    if (publishId) {
      await db.youTubePublish.updateMany({
        where: { id: publishId, userId: job.userId },
        data: { status: retryable ? "QUEUED" : "FAILED", error: message.slice(0, 1000) },
      });
    }
    ...
    await audit({ userId: job.userId, action: "YOUTUBE_PUBLISH_FAILED", resource: "YOUTUBE_PUBLISH", success: false, resourceId: publishId, metadata: JSON.stringify({ retryable, uploadStarted }) });
    return NextResponse.json({ error: "YouTube upload failed", retryable, attempts: job.attempts }, { status: retryable ? 503 : 500 });
  } finally {
    if (file) await removeMaterializedAsset(file).catch(() => undefined);
  }
}
