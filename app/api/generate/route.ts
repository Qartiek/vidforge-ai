...
export async function POST(request:Request){
 ...
 try{
  ...
  await db.auditLog.create({data:{userId:user.id,action:"CONTENT_GENERATED",resource:"Project",resourceId:project.id,success:true,metadata:JSON.stringify({jobId:job.id,platform:brief.platform,format:brief.format,researchAware:Boolean(brief.researchContext)})}});
  return NextResponse.json({projectId:project.id,contentScriptId:script.id,jobId:job.id,status:"queued",brief,content});
