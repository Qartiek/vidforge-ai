...
export async function GET(request: Request) {
  ...
  (await cookies()).delete("youtube_oauth_state");
  ...
  if (stateUserId !== user.id || !state || !cryptoSafeEqual(state, returnedState)) return NextResponse.json({ error: "Invalid OAuth state" }, { status: 400 });
  ...
  if (!token.refresh_token) return NextResponse.json({ error: "No refresh token returned; reconnect with consent" }, { status: 400 });
  ...
  await db.youTubeConnection.upsert({ where: { userId: user.id }, create: { userId: user.id, channelId: channel.id, channelTitle: channel.snippet?.title, accessToken: token.access_token ? encryptSecret(token.access_token) : null, refreshToken: encryptSecret(token.refresh_token), expiresAt: token.expires_in ? new Date(Date.now() + token.expires_in * 1000) : null, scope: token.scope }, update: { channelId: channel.id, channelTitle: channel.snippet?.title, accessToken: token.access_token ? encryptSecret(token.access_token) : undefined, refreshToken: encryptSecret(token.refresh_token), expiresAt: token.expires_in ? new Date(Date.now() + token.expires_in * 1000) : null, scope: token.scope } });
