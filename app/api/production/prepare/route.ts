...
export async function POST(request: Request) {
  ...
  const job = await db.job.create({ data: { userId: user.id, projectId: project.id, type: "media_production", status: "QUEUED", payload: JSON.stringify({ plan, scriptId: script.id }) } });
  await db.project.update({ where: { id: project.id }, data: { status: "PRODUCING" } });
  await db.auditLog.create({ data: { userId: user.id, action: "MEDIA_PRODUCTION_QUEUED", resource: "Job", resourceId: job.id, success: true } });
