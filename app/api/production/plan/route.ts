...
export async function POST(request: Request) {
  ...
  const existing = await db.job.findFirst({ where: { userId: user.id, projectId: project.id, type: "production", status: { in: ["QUEUED", "RUNNING"] } }, orderBy: { createdAt: "desc" } });
  if (existing) return NextResponse.json({ jobId: existing.id, status: existing.status.toLowerCase(), message: "Production plan already queued" }, { status: 202 });
  const plan = buildProductionPlan(script.script);
  const job = await db.job.create({ data: { userId: user.id, projectId: project.id, type: "production", status: "QUEUED", payload: JSON.stringify({ plan, scriptId: script.id }) } });
  await db.project.update({ where: { id: project.id }, data: { status: "PRODUCING" } });
  await db.auditLog.create({ data: { userId: user.id, action: "PRODUCTION_PLANNED", resource: "Job", resourceId: job.id, success: true, metadata: JSON.stringify({ durationSeconds: plan.durationSeconds, sceneCount: plan.scenes.length }) } });
