...
export async function POST(request: Request) {
  ...
  const rl = await rateLimit(`production:${user.id}`, 10, 3600);
  if (!rl.allowed) return NextResponse.json({ error: "Production rate limit exceeded" }, { status: 429 });

  const parsed = schema.safeParse(await request.json().catch(() => null));
  if (!parsed.success) return NextResponse.json({ error: "Invalid project request" }, { status: 400 });
  ...
  if (existing) {
    return NextResponse.json({
      ...
      message: "Production plan already queued",
    }, { status: 202 });
  }
  ...
  const job = await db.job.create({
    data: {
      ...
      type: "production",
      status: "QUEUED",
      payload: JSON.stringify({ plan, scriptId: project.scripts.id }),
    },
  });
  ...
  await db.auditLog.create({
    data: {
      ...
      resourceId: job.id,
      success: true,
      metadata: JSON.stringify({ durationSeconds: plan.durationSeconds, sceneCount: plan.scenes.length }),
    },
  });
