...
export async function createOptimizationRecommendations(snapshot: YouTubeAnalyticsSnapshot) {
...
export async function recordAnalyticsJob(userId: string, projectId: string, publishId: string, videoId: string) {
  const existing = await db.job.findFirst({
    where: { userId, projectId, type: "ANALYTICS_OPTIMIZATION", payload: { contains: `\"publishId\":\"${publishId}\"` }, status: { in: ["QUEUED", "RUNNING", "SUCCEEDED"] } },
  ...
  return db.job.create({
    data: {
      ...
      status: "QUEUED",
      payload: JSON.stringify({ publishId, videoId, provider: "youtube-analytics-api" }),
    },
  });
}
