...
export async function fetchYouTubeAnalytics(userId: string, videoId: string): Promise<YouTubeAnalyticsSnapshot> {
  ...
  return {
    ...
    likes: values[1] || 0,
    comments: values[2] || 0,
    watchTimeMinutes: values[3] || 0,
    averageViewDurationSeconds: values[4] || 0,
    averageViewPercentage: values[5] || 0,
...
export function createOptimizationRecommendations(snapshot: YouTubeAnalyticsSnapshot) {
  ...
  if (snapshot.averageViewPercentage > 0 && snapshot.averageViewPercentage < 35)
    recommendations.push("Retention is weak: tighten the opening, remove slow sections, and add stronger pattern interrupts earlier.");
  else if (snapshot.averageViewPercentage >= 50)
    recommendations.push("Retention is healthy: preserve the current pacing and opening structure in the next iteration.");
  ...
  if (snapshot.averageViewDurationSeconds > 0 && snapshot.views > 0)
    recommendations.push("Use the observed average view duration as the baseline when deciding the next video's target length.");
...
export async function recordAnalyticsJob(userId: string, projectId: string | null, publishId: string, videoId: string) {
  ...
  const existing = candidates.find((j) => {
    const p = JSON.parse(j.payload) as { publishId?: string };
    return p.publishId === publishId;
  });
  ...
  return db.job.create({
    data: {
      ...
      type: "ANALYTICS_OPTIMIZATION",
      status: "QUEUED",
      payload: JSON.stringify({ publishId, videoId }),
    },
  });
}
