...
export function runContentQualityChecks(input:{title?:string;script?:string;description?:string;tags?:string[]}):QualityReport{
 const script=input.script?.trim()||"";const title=input.title?.trim()||"";const description=input.description?.trim()||"";const tags=Array.isArray(input.tags)?input.tags.filter(Boolean):[];
 const opening=script.slice(0,900);const ending=script.slice(-1400);
