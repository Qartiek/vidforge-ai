...
export async function executePipelineStage(stage: string, projectId: string, scriptId: string) {
  ...
  if (stage === "render" || stage === "editing") {
    ...
    const manifest = { version: 2, format: "mp4", width: 1920, height: 1080, fps: 30, scenes: visualAssets.map((a, i) => ({ index: a.sceneIndex ?? i, assetUrl: a.url, motion: "ken-burns", transition: "crossfade" })), audio: { source: "tts", urls: audioAssets.map((a) => a.url), codec: "aac" }, captions: { enabled: true, source: "script" } };
  ...
  if (stage === "publish" || stage === "schedule_publish") {
    ...
    const job = await db.job.create({ data: { userId: project.userId, projectId, type: "YOUTUBE_PUBLISH", payload: JSON.stringify({ publishId: publish.id }) } });
