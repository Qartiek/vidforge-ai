...
async function uploadBinary(bytes: Buffer, filename: string, mimeType: string) {
  ...
  if (!body || typeof body.url !== "string") {
    throw new Error("Asset upload response did not contain a URL");
  }
  return body.url;
}
...
export async function executePipelineStage(stage: string, projectId: string, scriptId: string) {
  ...
  if (!project || !project.scripts || project.scripts.id !== scriptId) {
    throw new Error("Pipeline project/script mismatch");
  }

  if (stage === "voice") return runVoice(projectId, project.scripts.script);

  if (stage === "visuals") {
    const sentences = project.scripts.script
      .split(/(?<=[.!?])\s+/)
      .filter(Boolean)
      .slice(0, 12);
    const assets = [];
    for (let i = 0; i < sentences.length; i += 1) {
      assets.push(
        await runImage(
          projectId,
          `Cinematic business documentary visual, no text, factual corporate finance context. Scene: ${sentences[i]}`,
          "visual",
          i,
        ),
      );
  ...
  if (stage === "thumbnail") {
    const asset = await runImage(
      projectId,
      `High click-through YouTube thumbnail for a serious business finance video about ${project.topic}. Bold visual hierarchy, premium financial news aesthetic, dramatic lighting, one clear focal subject, minimal or no text.`,
      "thumbnail",
    );
  ...
  if (stage === "seo") {
    ...
    const generated = await generateContent(
      brief,
      buildScriptPrompt(brief, findings || project.scripts.script),
    );
  ...
  if (stage === "render") {
    const visualAssets = project.assets
      .filter((a) => a.type === "visual" || a.type === "image" || a.type === "video")
      .sort((a, b) => (a.sceneIndex ?? 0) - (b.sceneIndex ?? 0));
    const audioAssets = project.assets
      .filter((a) => a.type === "audio")
      .sort((a, b) => (a.sceneIndex ?? 0) - (b.sceneIndex ?? 0));
    if (!visualAssets.length || !audioAssets.length) {
      throw new Error("Render requires visual and voice assets");
    }
    const manifest = {
      version: 1,
      format: "mp4",
      ...
      height: 1080,
      fps: 30,
      scenes: visualAssets.map((a, i) => ({ index: a.sceneIndex ?? i, assetUrl: a.url })),
      audio: { source: "tts", urls: audioAssets.map((a) => a.url), codec: "aac" },
    };
    ...
    const row = await db.mediaAsset.create({
      data: {
        ...
        mimeType: "video/mp4",
        metadata: JSON.stringify({ rendered: true, manifestVersion: 1 }),
      },
    });
  ...
  if (stage === "publish") {
    const video = [...project.assets]
      .reverse()
      .find((a) => a.type === "video" && a.mimeType === "video/mp4");
    ...
    if (existing) return { publishId: existing.id, status: existing.status, duplicate: true };
    const privacy =
      process.env.PIPELINE_YOUTUBE_PRIVACY === "public"
        ? "public"
        : process.env.PIPELINE_YOUTUBE_PRIVACY === "unlisted"
          ? "unlisted"
          : "private";
    const publish = await db.youTubePublish.create({
      data: {
        ...
        projectId,
        idempotencyKey,
        title: project.scripts.title,
        description: project.scripts.description,
        tagsJson: project.scripts.tagsJson,
    ...
    return { publishId: publish.id, jobId: job.id, status: publish.status };
  }
