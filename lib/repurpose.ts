...
export async function generateRepurposedContent(input:RepurposeInput){
 ...
 const prompt=`You are NOVYN's platform repurposing engine. Convert the source into native short-form content. The hook MUST remain inside the opening of every final script. Do not invent unsupported facts. Adapt pacing, CTA, caption and hashtags to each platform. Supported platforms: YouTube, Instagram, Facebook, TikTok, LinkedIn, X. Return strict JSON only: {"items":[{"platform":"...","format":"Short|Reel|Video|Post","hook":"...","script":"...","caption":"...","hashtags":["..."]}]}. Create ${count} items per requested platform. Requested platforms: ${platforms.join(", ")}. Source title: ${input.title}. Source script:\n${input.script.slice(0,18000)}`;
